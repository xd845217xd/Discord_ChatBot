...
class ClearHistoryAndRestartCogOperation(Operation):
    async def execute(self):
        ...
        print(f"正在嘗試為 guild_id {guild_id} 清除對話紀錄...")
        ...
        if api_cog_name:
            try:
                ...
                if api_cog:
                    ...
                    await self.interaction.response.send_message(f"已清除 {api_cog_name} 的對話紀錄。", ephemeral=False)
            except Exception as e:
                await self.interaction.response.send_message(f"清除對話紀錄時出現錯誤: {str(e)}", ephemeral=True)
        else:
